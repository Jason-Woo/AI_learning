...
class W //存储向量w 
{
	public:
		...
		void Eva_W(set S); //评估evaluate值 
		void refresh(set S,W *best_w); //PLA算法更新w	
};
...
void W::refresh(set S,W *best_w) //best_w是口袋里最优的w 
{
	for(int i=0;i<S.index_num;i++)
	{
		double tmp_result=0;
...
int main()
{
	...
	set S1(train_set,0);
	W best_w(S1.index_dimension,1); //口袋中最优的w 
	best_w.Eva_W(S1);
